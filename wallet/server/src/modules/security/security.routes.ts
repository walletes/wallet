 ...
 * @route   GET /api/v1/security/scan
  * @desc    Scans for risky contract approvals (URL-based lookup)
   */
   router.get('/scan', validator.apiKeyAuth, validator.validateRequestBody, scanSecurityController);

   /**
   * @route   POST /api/v1/security/scan
   * @desc    Institutional Security Audit (JSON-body based)
   */
      router.post('/scan', validator.apiKeyAuth, validator.validateRequestBody, scanSecurityController);
